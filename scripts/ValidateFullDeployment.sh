...
sfdx force:source:deploy -p force-app --checkonly --testlevel NoTestRun --verbose -w 300 || exit 1
...
if grep -q '<types>' ./changed-sources/destructiveChanges/destructiveChanges.xml ; then
  ...
  #sfdx force:mdapi:deploy -d "changed-sources/destructiveChanges" --checkonly --ignorewarnings --verbose -w 300 || exit 1
else
